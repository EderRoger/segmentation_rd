...
RSpec.describe QueryBuilder, type: :model do
  ...
  it 'get by state and age' do
    ...
    expect(Contact.where(condition.each{|value| value }).size).to eq(1)
  end
  
  it 'get by state' do
    query = QueryBuilder.create(name: "Find by state Sao Paulo", condition: '[{"field": "state","operator": "=", "value": "Sao Paulo"}]')
    condition = query.builder
    expect(Contact.where(condition.each{|value| value }).size).to eq(2)
  end

  it 'get by role' do
    query = QueryBuilder.create(name: "Find by role Manager", condition: '[{"field": "role","operator": "=", "value": "Manager"}]')
    condition = query.builder
    expect(Contact.where(condition.each{|value| value }).size).to eq(2)
  end
end
